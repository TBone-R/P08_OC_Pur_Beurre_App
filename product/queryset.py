...
def get_order(user):
    ...
    if user.is_authenticated:
        pref = UserProfile.objects.get_or_create(
            user=user,
            defaults={
                "user": user,
        ...
        order.append(pref[0].order_by_1)
        order.append(pref[0].order_by_2)
        order.append(pref[0].order_by_3)
        order.append(pref[0].order_by_4)
    else:
